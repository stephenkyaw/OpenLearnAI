    ...
    return (
        <DashboardLayout>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                <StatCard icon={<BookOpen className="text-blue-600" />} label="Active Courses" value="12" sub="Currently Enrolled" />
                <StatCard icon={<CheckCircle className="text-emerald-600" />} label="Completed Courses" value="4" sub="Fully Finished" />
                <StatCard icon={<Award className="text-purple-600" />} label="Certificates" value="3" sub="Earned" />
            </div>

            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Main Action Area */}
                <div className="lg:col-span-2 space-y-8">
                    {/* Resume Hero - Solid White Card */}
                    <div className="bg-white rounded-3xl p-8 relative overflow-hidden group hover:shadow-xl transition-all duration-500 shadow-sm border border-slate-100">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-blue-50 rounded-full blur-3xl -mr-10 -mt-10 group-hover:bg-blue-100 transition-colors" />

                        <div className="relative z-10">
                            <div className="flex items-center justify-between mb-6">
                                <div className="flex items-center space-x-2 text-primary text-sm font-semibold bg-blue-50 px-3 py-1 rounded-full border border-blue-100">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                        ...
                        <div className="flex items-center justify-between text-sm mb-2">
                            ...
                            <span className="flex items-center text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full text-xs font-medium"><div className="h-1.5 w-1.5 bg-emerald-500 rounded-full mr-1.5" /> Operational</span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                            <span>AI Tutor</span>
                            <span className="flex items-center text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full text-xs font-medium"><div className="h-1.5 w-1.5 bg-emerald-500 rounded-full mr-1.5" /> Active</span>
                        </div>
                    </div>
                </div>
            </div>
...
function StatCard({ icon, label, value, sub }: { icon: React.ReactNode, label: string, value: string, sub: string }) {
    return (
        <div className="bg-white rounded-3xl p-6 flex items-center gap-6 hover:shadow-xl transition-all duration-300 group shadow-sm border border-slate-100">
            <div className="h-14 w-14 rounded-2xl bg-slate-50 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform duration-300 border border-slate-100">
                {icon}
            </div>
            <div>
...
function ActionCard({ icon, label }: { icon: React.ReactNode, label: string }) {
    return (
        <div className="bg-white rounded-2xl p-4 flex flex-col items-center justify-center text-center gap-3 hover:translate-y-[-2px] hover:shadow-lg transition-all duration-300 cursor-pointer group h-32 shadow-sm border border-slate-100">
            <div className="h-10 w-10 rounded-full bg-primary/5 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors duration-300">
                {icon}
            </div>
...
function CourseProgressCard({ title, module, progress, color, icon }: { title: string, module: string, progress: number, color: string, icon: React.ReactNode }) {
    return (
        <div className="bg-white rounded-2xl p-4 flex items-center gap-5 shadow-sm border border-slate-100 hover:shadow-md transition-all cursor-pointer group">
...
function ActivityItem({ title, sub, time }: { title: string, sub: string, time: string }) {
    return (
        <div className="flex items-start gap-3 p-2 hover:bg-white/50 rounded-xl transition-colors cursor-default">
...
function RecentCourseCard({ title, lastAccess }: { title: string, lastAccess: string }) {
    return (
        <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all cursor-pointer group flex items-center gap-3">
